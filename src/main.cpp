...
void drawItem(menu_item_t item, bool selected, uint8_t line) {
  u8g2.setFont(u8g2_font_t0_13b_tf);
  if (selected) {
    u8g2.drawUTF8(0, (line + 1) * FONT_HEIGHT_PX - FONT_GLYPH_LEG_PX + (line + 1) * LINE_SPACING_PX, ">");
  }
  u8g2.drawUTF8(8, (line + 1) * FONT_HEIGHT_PX - FONT_GLYPH_LEG_PX + (line + 1) * LINE_SPACING_PX, item.label);
  if (item.type == MENU_ITEM_TYPE_VALUE) {
    char value_str[10];
    dtostrf(*(item.target.value_ptr)/10.0, 0, 1, value_str);
    u8g2.drawUTF8(128 - u8g2.getUTF8Width(value_str), (line + 1) * FONT_HEIGHT_PX - FONT_GLYPH_LEG_PX + (line + 1) * LINE_SPACING_PX, value_str);
  }
}
...
void drawScreen(menu_screen_t screen, uint8_t cursor) {
  ...
  if (cursor == screen.item_count-1) {
    ...
    for (size_t i = cursor-1; i < cursor+2; i++) {
      drawItem(screen.items[i], i == cursor, i+1);
    }
  }
}
